...
namespace HabitTracker.WebApp.Features.Core.Config;

/// <summary>Typed key/value settings store backed by <see cref="DbConfigEntry"/>. Each entry records a
/// type tag so reads validate against the stored type. Primitives, dates and strings are stored in their
/// invariant string form; everything else is JSON.</summary>
[ScopedService]
