...
/// <summary>Replaces the running app container with one based on the latest image. Because a container
/// cannot recreate itself, this runs a short-lived <c>docker:cli</c> sidecar (with the Docker socket
/// mounted) that stops, removes and re-runs the app container with the same configuration.</summary>
[SingletonService]
public class UpdateExecutor
{
    ...
    public async Task Execute()
    {
        ...
        await _dockerService.RunContainer(new RunContainerRequest(
            ...
            Labels: new Dictionary<string, string> { ["habittracker.sidecar"] = "update" }.ToImmutableDictionary()));
    }
